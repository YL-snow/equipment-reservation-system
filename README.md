...
| 设备信息展示 | 列表展示型号、分类、总库存、可用库存；库存低于阈值时显示库存预警（独立详情页暂未开放） |
...
| 冲突检测 | 数据库行锁 + 原子库存更新 + 乐观锁版本号，确保同一时段不重复预约、并发审批不超卖 |
| 预约审批 | 管理员通过/驳回预约申请 |
| 设备归还 | 确认设备归还，自动恢复库存 |
...
| 密码管理 | 修改密码、管理员重置密码 |
| 角色管理 | 后端提供角色修改接口（ADMIN / STUDENT），前端管理页暂未开放 |
...
| 并发控制 | 悲观锁 + 原子更新 vs 单纯乐观锁 | 审批和库存变化使用数据库行锁与原子更新防超卖，`@Version` 版本号兜底防脏写 |
...
- **冲突检测正确性**：使用数据库悲观锁 + 原子库存更新（`WHERE availableQty >= quantity`）+ `@Version` 版本号，确保并发场景下预约不超卖
